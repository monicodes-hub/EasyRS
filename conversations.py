...
def main(file_path=None, agent_name=None, conv_mode=None):
    ...
    # 6. For each row with the selected agent, extract the CHATID from the same CHANNEL cell.
    chatids = set()
    for _, row in agent_rows.iterrows():
        channel_value = str(row["CHANNEL"])
        match = re.search(r"CHATID:\s*([^,\s]+)", channel_value)
        if match:
            chatids.add(normalize_id(match.group(1)))
